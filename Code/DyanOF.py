...
class Encoder(nn.Module):
    def __init__(self, dyan_lambda, Drr, Dtheta, T, gpu_id):
        ...
        self.T = T
        self.gid = gpu_id
        self.dyan_lambda = dyan_lambda

    def forward(self, x):
        dic = creatRealDictionary(self.T,self.rr,self.theta,self.gid)
        ## for UCF Dataset:
        sparsecode = fista(dic,x,self.dyan_lambda,100,self.gid) # original DYAN LAMBDA was 0.1
...
class OFModel(nn.Module):
    def __init__(self, dyan_lambda, Drr, Dtheta, T, PRE, gpu_id):
        super(OFModel, self).__init__()
        self.l1 = Encoder(dyan_lambda, Drr, Dtheta, T, gpu_id)
